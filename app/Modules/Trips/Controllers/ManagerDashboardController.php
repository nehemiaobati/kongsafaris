...
class ManagerDashboardController extends BaseController
{
     ...
     *
     * Handles full edits including addresses, vehicle/driver reassignment,
     * pricing, payment status, and trip status changes.
     */
    public function updateBooking(): ResponseInterface
    {
        ...
        try {
            ...
            if (in_array($new_trip_status, ['cancelled', 'completed'], true) && $old_trip_status === 'active') {
                $driver = $driverModel->find($old_driver_id);
                if ($driver !== null) {
                    $driverModel->update($driver->id, ['status' => 'available']);
                }
            }
            ...
            if ($new_trip_status === 'active' && $old_trip_status !== 'active') {
                $newDriver = $driverModel->find($new_driver_id);
                if ($newDriver !== null) {
                    $driverModel->update($newDriver->id, ['status' => 'on_trip']);
                }
            }
            ...
            if ($new_trip_status === 'active' && $old_driver_id !== $new_driver_id) {
                $oldDriver = $driverModel->find($old_driver_id);
                if ($oldDriver !== null) {
                    $driverModel->update($oldDriver->id, ['status' => 'available']);
                }
                $newDriver = $driverModel->find($new_driver_id);
                if ($newDriver !== null) {
                    $driverModel->update($newDriver->id, ['status' => 'on_trip']);
